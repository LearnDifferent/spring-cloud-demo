...
- 比如，使用本地日志表来记录事务提交前后的数据，通过消息队列（MQ）来辅助控制事务，异步将数据同步
- 在这种情况下，因为数据不是即时同步的，就存在因为网络延迟/超时导致需要重发的可能性。如果重发了，就可能多次消费，所以要考虑幂等性
...
解决分布式事务问题，各个子系统之间必须能感知到彼此的事务状态，才能保证状态一致，因此需要一个 **事务协调者** 来协调每一个事务的参与者（子系统的事务）。
...
- 每个数据库参与者在本地执行事务，并写本地的 Undo Log
- 此时事务还未提交
...
>[AT 模式 - 官方文档](https://seata.apache.org/zh-cn/docs/dev/mode/at-mode/)
...
在 TC 发出指令要提交的时候，服务（TM 和 RM）可以先回复 ACK，然后将这个指令加入到一个 Blocking Queue 中，再通过一个线程去轮询（schedule）本地日志以完成指令。

### AT 模式的“一阶段”


1. 解析 SQL：拦截并解析 SQL，得到 SQL 的类型（UPDATE），表（product），条件（where name = 'TXC'）等相关的信息
2. 查询 **前镜像** ：根据解析得到的条件信息，生成查询语句，定位数据，得到前镜像
3. 执行业务 SQL
4. 查询 **后镜像** ：根据前镜像的结果，通过 **主键** 定位数据，得到后镜像
5. 插入回滚日志：把前后镜像数据以及业务 SQL 相关的信息组成一条回滚日志记录，插入到 `UNDO_LOG` 表中
6. 提交前，向 TC 注册分支：申请需要被修改的数据的 **全局锁** （通过主键定位该条记录）
...
4. 根据 UNDO LOG 中的前镜像和业务 SQL 的相关信息，生成并执行回滚的语句（反向的 SQL 更新语句）
...
在 RM 进行本地事务的提交，但是全局事务还在进行的过程中，因为本地事务已经提交了，数据就变为最新的了，这个时候如果全局事务还没提交/回滚，那么就会出现 *脏读* 。

为了解决脏读，需要使用 `select ... for update` 先去锁住数据，实现 *读隔离* 。

在加了 `for update` 后，Seata 中的 data source 是原来的 data source 的代理，所以也会触发全局锁，实现读隔离，让其他的操作无法执行。

	不过这个要看情况，有时候不需要读隔离就别这样做。

## TCC 模式（对比 AT 模式）

TCC：Try、Confirm、Cancel

>下文来自 [Seata TCC 模式](https://seata.apache.org/zh-cn/docs/dev/mode/tcc-mode/)
...
- 向前恢复 Forward Recovery：
	- 对于执行不通过的事务，会尝试重试事务。如果需要事务的恢复，也是按照流程顺序往前恢复。
	- 比如：支付宝的回调。如果成功就 return SUCCESS；如果没有 return 结果，就使用衰减回调，第一次 15s，第二次 30s，第三次 5min 这样不停重试（Retry）。
	- 除了执行的时候需要重试，恢复的时候如果失败也是重试。
	- 【流程1->流程2->流程3->……->流程N】这样产生的数据，在恢复的时候，就从【流程N->……->流程3->流程2->流程1】这样向前一个流程的数据逐步恢复。如果恢复的时候失败了，就重试。
- 向后恢复 Backward Recovery：
	- 在执行事务失败时，补偿所有已完成的事务
	- 这意味着，每个事务都要有一个冲正的接口
	- 比如：银行的“冲正”，如果发生错误，不是将之前的记录抹掉，而是形成一条“冲正”的记录，对错误的业务进行纠错。

## XA 模式

>[下文来自官网](https://seata.apache.org/zh-cn/docs/user/mode/xa)
